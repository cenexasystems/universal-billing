...
export const CreateBarcodeModal: React.FC<CreateBarcodeModalProps> = ({
  ...
  const selectProductItem = useCallback(async (prod: ProductOption, targetVariantId?: string | null) => {
    ...
    const code = prod.barcode || `Universal Look${Math.floor(1000000 + Math.random() * 9000000)}`
    setItemCode(code)
    setLine1(prod.name)
  ...
  useEffect(() => {
    const codeToRender = (itemCode && itemCode.trim()) || 'Universal Look0000000'
  ...
  const handleAssignCode = () => {
    const generated = 'Universal Look' + Math.floor(1000000 + Math.random() * 9000000)
      ...
      for (let i = 0; i < count; i++) {
        allStickers.push(`
          <div class="label-sticker">
            ...
            <div class="footer">
              <span>${item.line2 ? `<span class="tag">${item.line2}</span>` : '<span class="tag">Universal Look RETAIL</span>'}</span>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const itemsToAdd: BarcodeQueueItem[] = unassignedVariants.map((v) => ({
                              ...
                              barcodeValue: `Universal Look${Math.floor(1000000 + Math.random() * 9000000)}`,
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {itemCode || 'Universal Look0000000'}
                          </span>
