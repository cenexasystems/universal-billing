...
export const InventoryAnalyticsView: React.FC = () => {
  ...
  const exportSnapshotCsv = async () => {
    ...
    try {
      ...
      link.setAttribute('download', `Universal Look_Inventory_Snapshot_${new Date().toISOString().slice(0, 10)}.csv`)
  ...
  const exportCsv = () => {
    ...
    const link = document.createElement('a')
    link.setAttribute('href', encodedUri)
    link.setAttribute('download', `Universal Look_Inventory_Movements_${range}_${Date.now()}.csv`)
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  }
